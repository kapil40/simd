...
def get_parms():
    ...
    disk_throughput = float(1.0)  # Default value
    repair_bandwidth_percentage = float(0.1)  # Default value
    disk_size = float(1024.0)  # Default value in GB
    array_size = int(10)  # Default value
    ...
    try:
        (opts, args) = getopt.getopt(sys.argv[1:], "hl:m:i:r:n:c:p:F:R:L:S:T:P:D:A:a:t:fdwo:", ["help", "log", "mission_time", 
                                                                             "iterations",
                                                                             "raid", "raid_num", 
                                                                             "capacity", 
                                                                             "parameters", 
    ...
    for o, a in opts:
        ...
        elif o in ("-T", "--disk_throughput"):
            disk_throughput = float(a)

        elif o in ("-P", "--repair_bandwidth_percentage"):
            repair_bandwidth_percentage = float(a)

        elif o in ("-D", "--disk_size"):
            disk_size = float(a)

        elif o in ("-A", "--array_size"):
            array_size = int(a)
...
# check weibull with alex suggstion
# update the output and add arguments